...
BLOCK = os.urandom(1024 * 256)  # 256KB of random bytes, reused to build larger files fast
...
def main():
    ...
    # A large "hot" file — recently touched, should NOT be touched by AegisStore
    make_file(target / "active_project" / "current_work.db", 50 * 1024 * 1024, age_days=0.2)

    # An old, large, duplicated dataset — classic Cold + Redundant candidate
    ...
    # Old build artifacts — Cold, not duplicated
    ...
    # A moderately old file with lower confidence (Warm territory) — should land as MEDIUM
